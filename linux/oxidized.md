...
<br>
<br> Как можно обратить внимание, в конфиге указан запуск только локалхост с портом 8888.
Это сделано специально, так как установка паролей для пользователя не предусмотрено.
...
Проверяем внесены ли исключения в firewall
...
Теперь nginx будет работать на стандартном порту (или на том, который вы укажете в его настройках), при обращении к нему будет происходить авторизация и пользователь будет перенаправлен на адрес proxy_pass (127.0.0.1:8888 в нашем случае).<br><br><br>
